...
namespace Lucene.Net.Linq
{
    ...
    {
        private readonly ILogger Log = Logging.CreateLogger<LuceneSession<T>>();
        ...
        public void Commit()
        {
            lock (sessionLock)
            {
                ...
                lock (context.TransactionLock)
                {
                    ...
                    {
                        Log.LogError(ex, "OutOfMemoryException while writing/committing to Lucene index. Closing writer.");
                        try
                        {
                            writer.Dispose();
                        }
                        ...
                        {
                            Log.LogError(ex2, "Exception in IndexWriter.Dispose");
                    ...
                    {
                        Log.LogError(ex, "Exception in commit");
                        try
                        {
                            writer.Rollback();
                        }
                        ...
                        {
                            Log.LogError(ex2, "Exception in rollback");
                            throw new AggregateException(ex, ex2);
                        }
        ...
        private void CommitInternal(IList<Document> additions)
        {
            ...
            {
                deletes.Apply(query => Log.LogTrace("Delete by query: {Query}", query));
            ...
            Log.LogDebug("Applied {DeleteCount} deletes and {AdditionCount} additions.", deletes.Count(), additions.Count);
            ...
            Log.LogInformation("Commit completed.");
        }
        ...
        internal IList<Document> StageModifiedDocuments()
        {
            ...
            {
                var captured = doc;
                Log.LogTrace("Flushing modified document {Document}", captured);
